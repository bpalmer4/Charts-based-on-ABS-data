...
# public
def get_plot_constants(meta:pd.DataFrame
    ) -> tuple[pd.Timestamp, list, list]:
    ...
    reasonable_end = meta['Series End'][meta['Series End'] <= pd.Timestamp('today')]
    reasonable_end = reasonable_end.max() if len(reasonable_end) > 0 else pd.Timestamp('today')
    recent = (
        reasonable_end
...
# private
def _get_zip_from_cache(url: str | list[str],
    ...
    with zipfile.ZipFile(cache_path, 'w', zipfile.ZIP_DEFLATED) as zfile:
        for name, contents in xl_dict.items():
            if not contents:
                ...
                continue
            zfile.writestr(f'/{name}', contents, )
...
# private
def _get_abs_zip_file(catalogue_id: str, table: int, verbose: bool
                     ) -> bytes | None:
...
# private
def _get_meta(excel: pd.ExcelFile, tab_num: str, tab_desc: str,
    ...
    file_meta = excel.parse('Index', header=9, parse_dates=True,
...
# private
def _get_data(excel: pd.ExcelFile, meta: pd.DataFrame, freq: str,
    ...
    for sheet_name in data_sheets:
        sheet_data = excel.parse(sheet_name, header=9, index_col=0,
    ...
    with zipfile.ZipFile(io.BytesIO(zip_file)) as zipped:
        for element in zipped.infolist():
            ...
            if 'Index' not in excel.sheet_names:
                print(f'Caution: Could not find the "Index" sheet in {element.filename}')
                continue
...
# public
def get_ABS_meta_and_data(catalogue_id: str, table: int=0,
                          verbose: bool=False
                         ) -> None | dict[str, pd.DataFrame]:
...
def find_id(meta:pd.DataFrame, search_terms: dict[str, str],
    ...
    for phrase, column in search_terms.items():
        ...
        if exact or column == 'Table': # always match the table column exactly
            ...
            meta_select = meta_select[meta_select[column].str.contains(phrase, regex=False)]
    if verbose:
...
# public
def get_identifier(meta: pd.DataFrame,
