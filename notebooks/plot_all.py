...
class Tudds:
    """Vehicle for remembering some key information."""
...
def plotall(cat_id: str) -> None:
    ...
    source, chart_dir, _, meta = get_fs_constants(
        abs_dict, landing_page, " - ALL"
    )
    ...
    for table in meta[metacol.table].unique():
        ...
        for unit in sub_set1[metacol.unit].unique():
            ...
            for dtype in sub_set2[metacol.dtype].unique():
                ...
                plot_subset(sub_set3, Tudds(table, unit, dtype, abs_dict, landing_page, source))
...
def plot_subset(subset: pd.DataFrame, tudds: Tudds) -> None:
    ...
    for did in subset[metacol.did].unique():
        ...
        elif (SEAS_ADJ in selected[metacol.stype].unique() 
...
def title_fix(title: str) -> str:
    ...
    roughly in half over two lines."""
    ...
    if n_folds > 1:
        for fold in range(1, n_folds):
            ...
            breaks = {abs(int(length * fold / n_folds) - p): i for i, p in enumerate(positions)}
